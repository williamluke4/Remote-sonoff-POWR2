...
const char *wifiSSID = "Salaam";
const char *wifiPassword = "20Palmtree18";
...
const char *mqtt_server = "192.168.1.244"; // Replace with your MQTT broker IP
const int mqtt_port = 1883;
const char *mqtt_topic = "powr2/data";
const char *mqtt_command_topic = "powr2/command";

#define ONE_WIRE_BUS 4 // IO4 pin

OneWire oneWire(ONE_WIRE_BUS);
DallasTemperature sensors(&oneWire);

// Add this global variable to store the temperature setpoint
float temperatureSetpoint = 30.0;
...
void mqtt_callback(char *topic, byte *payload, unsigned int length)
{
  if (strcmp(topic, mqtt_command_topic) == 0)
  {
    ...
    {
      temperatureSetpoint = command.substring(9).toFloat();
      // Store the temperature setpoint in EEPROM or flash memory if you want it to persist across reboots
      // For example: EEPROM.put(0, temperatureSetpoint);
      // EEPROM.commit();
    }
  }
}
void publishSensorData()
{
  ...
  String jsonData = "{";
  jsonData += "\"voltage\":" + String(_cseSensor.getVoltage()) + ",";
  jsonData += "\"current\":" + String(_cseSensor.getCurrent()) + ",";
  jsonData += "\"active_power\":" + String(_cseSensor.getActivePower()) + ",";
  jsonData += "\"apparent_power\":" + String(_cseSensor.getApparentPower()) + ",";
  jsonData += "\"reactive_power\":" + String(_cseSensor.getReactivePower()) + ",";
  jsonData += "\"energy\":" + String(_cseSensor.getEnergy()) + ",";
  jsonData += "\"temperature\":" + String(temperature) + ",";
  jsonData += "\"temperature_setpoint\":" + String(temperatureSetpoint) + ",";
  jsonData += "\"relay_state\":" + String(relayState);
  jsonData += "}";
...
void setup()
{
  ...
  _server.on("/info", []()
             { _server.send(200, "text/plain", "Sonoff POWR2"); });
  ...
  // If you want to persist the temperature setpoint across reboots, read it from EEPROM or flash memory
  // For example: EEPROM.get(0, temperatureSetpoint);
}
...
void loop()
{
  ...
  else if (currentTemperature < temperatureSetpoint - 1)
  { // Add hysteresis
    relayState = true;
    digitalWrite(RELAY_PIN, HIGH);
  }
...
void handelGetData()
{
  ...
  jsonData += "\"},";
  jsonData += "{\"dataValue\":\"";
  jsonData += String(temperatureSetpoint);
  jsonData += "\"},";
  jsonData += "{\"dataValue\":\"";
